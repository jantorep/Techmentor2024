param (
  ...
  [Parameter()]
  [String]$TemplateParameterFile = "config\custom-parameters\resourceGroupPrivateDNS.parameters.all.json",
...
$inputObject = @{
  DeploymentName        = 'alz-PrivateDNSRGDeploy-{0}' -f ( -join (Get-Date -Format 'yyyyMMddTHHMMssffffZ')[0..63])
  Location              = $Location
