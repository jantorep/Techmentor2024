...
param (
  ...
  [Parameter()]
  [String]$TemplateParameterFile = "config\custom-parameters\resourceGroupManagement.parameters.all.json",
...
$inputObject = @{
  DeploymentName        = 'ManagementRGDeploy-{0}' -f ( -join (Get-Date -Format 'yyyyMMddTHHMMssffffZ')[0..63])
  Location              = $Location
